  ...
  return (
    <article className="space-y-10">
      ...
      <section className="space-y-4 rounded-3xl border border-slate-800 bg-slate-950/80 p-6 shadow-lg shadow-slate-950/60">
        ...
        <p className="text-sm leading-relaxed text-slate-200 sm:text-base">
          Spring zwischen Weapon-Guides, um Rollen und Spielgefühl schnell zu vergleichen – und nutze die Waffen-Tierliste, wenn du einen Meta-Überblick willst.
        </p>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {weapons
            .filter((other) => other.id !== weapon.id)
            .map((other) => (
              <Link
                key={other.id}
                href={`${base}/guides/weapons/${other.id}`}
              ...
              >
                ...
                <p className="mt-1 text-xs text-slate-400">
                  Rolle: {roleLabelMap[other.role] ?? other.role}
                </p>
              </Link>
            ))}
        </div>
