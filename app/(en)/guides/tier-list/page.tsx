...
const julyChangeNotes = [
  {
    title: "Version 2.0 freshness check",
    detail:
      "This Where Winds Meet tier list was rechecked for August 2026 after the Version 2.0 update. July 23 Path Balance and Arena changes mean older placements are a baseline to retest, not a permanent verdict.",
    href: "/news#hidden-mountain-watch",
    label: "Latest update watch",
  },
  {
    title: "PvE and PvP split",
    detail:
      "Broad tier-list searches need separate PvE comfort, PvP arena pressure, and weapon-only comparisons instead of one flat ranking.",
    href: "/guides/pvp-tier-list",
    label: "PvP tier list",
  },
  {
    title: "Video freshness support",
    detail:
      "Use the fresh video page to see current update coverage and creator reactions before treating older launch-era rankings as final.",
    href: "/videos",
    label: "2026 videos",
  },
];
...
export const metadata: Metadata = {
  ...
  description:
    "Where Winds Meet tier list for August 2026 and Version 2.0: best PVE weapons, PVP arena ranks, builds, paths, Gauntlets, and balance checks.",
  ...
  return (
    <>
      ...
      <section id="what-changed" className="mb-6 rounded-2xl border border-violet-400/30 bg-violet-500/10 p-5 text-sm text-slate-200 shadow-sm shadow-violet-950/40">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-wide text-violet-200">Updated for August 2026</p>
            ...
            <p className="mt-3 leading-6 text-slate-300">
              ...
              and weapon rankings move quickly from a broad answer into the right specialist page.
            </p>
          </div>
          <span className="rounded-full border border-violet-300/50 px-3 py-1 text-xs font-semibold text-violet-100">
            Ranking context, not patch notes
          </span>
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {julyChangeNotes.map((item) => (
            <Link key={item.title} href={item.href} className="rounded-xl border border-slate-800 bg-slate-950/70 p-4 transition hover:border-violet-300/60">
              ...
              <p className="mt-3 text-xs font-semibold text-violet-200">{item.label}</p>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section id="arena-ranks" className="mb-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-5 text-sm text-slate-200 shadow-sm shadow-emerald-950/40">
        ...
        <p className="mt-3 leading-6 text-slate-300">
          ...
          fits the Version 2.0 live environment and the latest confirmed Path Balance baseline. Because the July 23 pass changed multiple Paths and Arena interactions, retest any older ranking before a costly respec.
        </p>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
